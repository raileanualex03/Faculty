class directedGraph:
    ...
    def KruskalAlgorithm(self):
        ...
        # sorting the list increasing by each edge's cost
    ...
    def canGo(self, v, pos, path):
        ...
        # by safe I mean either it is an edge inbetween them
    ...
    def hamiltonianCycleUtil(self, path, pos):
        if pos == self.__vertices:
            if self.isEdge(path[pos-1], path[0] == True) or self.isEdge(path[0], path[pos-1]) == True:
                return True
            else:
                return False
